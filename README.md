...
Execute this command to export the database named "COFFEE" from a MySQL server running on the host with DNS address "mysql-host-ip-address" on port 3306. The command is executed using the "mysqldump" tool and the output is saved to the file "my_sql.sql" in a parent directory of the current directory. The user connecting to the MySQL server is "nodeapp" and the password is prompted for during the execution of the command. Don't forget to replace the <mysql-host-ip-address> with yours.
```
